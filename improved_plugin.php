...
class Clawd_WP_API_Connector {
    ...
    private function set_featured_image($post_id, $image_url) {
        ...
        // استخدام cURL بدلاً من file_get_contents للتوافق الأفضل
        $image_data = $this->fetch_image_with_curl($image_url);
        
        if (!$image_data) {
            // إذا فشل التحميل، نحفظ فقط رابط الصورة كميتا بيانات
        ...
        if (strpos($filename, '.') === false) {
            $filename .= '.jpg'; // إضافة امتداد افتراضي إذا لم يكن موجودًا
        }
        ...
        // التحقق مما إذا تم تعيين الصورة البارزة بنجاح
        if (!$result) {
            update_post_meta($post_id, 'external_featured_image_url', $image_url);
        }
    ...
    /**
     * تحميل الصورة باستخدام cURL للتوافق الأفضل
     */
    private function fetch_image_with_curl($url) {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (compatible; ClawdBot)');
    ...
    public function options_page() {
        ...
        <div class="wrap">
            ...
            <h3>معلومات فنية</h3>
            <p>الإصدار الحالي: 1.0.1 - تحسينات في التعامل مع الصور البارزة</p>
        </div>
        <?php
    }
}
